...
function cleanup
{
    ...
    rm -rf {./dockersetup/main.py,./dockersetup/dist,./dist}
...
then
    ...
    rsync -a ./src/main.py dockersetup/
    # rsync -a .env dockersetup/
    cd ./dockersetup

    # build docker image
    docker build --no-cache -t hotspots-api:${IMAGE_TAG} --build-arg WHL_FILENAME=${WHL_FILE} .
    echo -e "\n##############################################################"
